...
const List: GridComponents["List"] = React.forwardRef(
  ({ style, children, ...props }, ref) => (
    <div
      ...
      {...props}
      ref={ref}
      className="grid grid-cols-1  w-full h-fit   scroll-smooth gap-y-3   p-0 m-0"
    >
      {children}
    </div>
  ),
        ...
        <VirtuosoGrid<IVideo>
          ...
          itemContent={(_, data) => (
            <ListVideoCard
              ...
              isSuccess={isSuccess}
              lazyLoading={true}
              avatarClassName={` flex flex-col  justify-center  w-3/5 text-[12px]  ${avatarClassName}`}
            />
          )}
          endReached={() => {
            if (hasNextPage) {
