  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent
          className=" z-50 gap-1.5 flex flex-col text-sm font-semibold tracking-tight text-foreground  bg-accent p-2 rounded-sm h-fit w-fit"
          align="start"
          sideOffset={10}
        >
          <DropdownMenuLabel className="text-md">Profile</DropdownMenuLabel>
          <DropdownMenuSeparator className="bg-foreground/50 my-0.5" />
          {isAuthenticated ? (
            <>
              <DropdownMenuGroup className="flex flex-col justify-center pl-5 ">
                <DropdownMenuItem>
                  <Link to="/dashboard">Dashboard</Link>
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <Link to={`/channel/home?username=${user.username}`}>
                    Home
                  </Link>
                </DropdownMenuItem>
                {isMobile && (
              ...
              <DropdownMenuGroup className="flex flex-col justify-center pl-5">
                <DropdownMenuSeparator className="-ml-5 bg-foreground/50 my-0.5" />
                <DropdownMenuItem>
                  <Link to={"/settings/channel"}>Channel</Link>
                </DropdownMenuItem>
                <DropdownMenuItem>
                  <Link to="/settings/video">Video</Link>
                </DropdownMenuItem>
              </DropdownMenuGroup>
              <DropdownMenuItem>
